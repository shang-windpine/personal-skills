# Domain Discovery (Backend)

How to find the business capabilities of an undocumented backend service, and which to map first. Frontend counterpart: [frontend-discovery.md](frontend-discovery.md). Findings land in the unified `L1-capabilities.md`, refining the seed inventory.
...
- **Layered packaging masquerading as structure.** If top-level packages are `controller` / `service` / `repository` / `dto`, the package tree tells you nothing about the domain and you must use the other signals below. If they are named after domain areas, the tree is your first draft of the capability list — but verify it, since package names drift from reality.
...
**1. The external API surface.** Every route registration, grouped by path prefix, is close to a literal capability list — it is what the outside world is allowed to ask for. Find them via the router registrations in [stack-probes.md](stack-probes.md), or from an OpenAPI spec if one exists. If a dedicated public-API module exists, group its registrations separately: they are the `PUBLIC` surface (see [stitching.md](stitching.md)).

**2. Message and event names.** MQ exchange/queue names and EventBus addresses are frequently named after business events. This is the highest-density source of domain vocabulary in the whole repo, and it also reveals capabilities that have no HTTP surface.
...
1. **The acceptance use cases**, if the KB defines them ([acceptance-protocol.md](acceptance-protocol.md)), until they pass — they double as the money path.
2. **Whatever you have been assigned.** Immediate need beats completeness, always.
